...
pub fn check_file_hash(file_data: &Vec<u8>, expected_hash: &[u8; 20]) -> std::io::Result<()>{
        let mut hasher = Sha1::new();
        hasher.update(file_data.clone());
...
pub fn imagenet_check_models(base_path: &PathBuf)-> std::io::Result<OpenvinoModel> {
    let imagenet_path = base_path.join("imagenet");
    let model_xml = fs::read(&imagenet_path.join("model.xml"))?;
    check_file_hash(&model_xml, &hex!("380a4621bf51ae357cb0eaafab203f214dbb036c"))?;
    let model_weights = fs::read(&imagenet_path.join("model.bin"))?;
    check_file_hash(&model_weights, &hex!("a50b3bbd47369e306002193fd18847a186c0bcf4"))?;
    Ok(OpenvinoModel{
        xml: model_xml,
...
pub fn imagenet_download(base_path: &PathBuf) -> Result<(), anyhow::Error>  {
